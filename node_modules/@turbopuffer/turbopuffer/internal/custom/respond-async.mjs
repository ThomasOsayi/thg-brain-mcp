...
export async function maybePoll(client, response, requestURL, options, clock) {
...
function extractLocation(response, origURL) {
...
function resolvePollResponse(body, clock) {
    ...
    if (body.result?.error) {
        ...
        throw Errors.APIError.generate(err.status_code, err.detail, undefined, new Headers());
    }
    throw new Errors.TurbopufferError('malformed poll response');
}
function respondAsyncApplied(response) {
...
//# sourceMappingURL=respond-async.mjs.map